...
public class Drum : MonoBehaviour
{
    ...
    private void Start()
    {
        int bulletsInDrum = PlayerPrefs.GetInt("BulletsInDrum");
        CurrentBullet = (bulletsInDrum == 0 || bulletsInDrum == MaxBulletsAmount) ? 0 : bulletsInDrum;
    ...
    private void Update()
    {
        transform.rotation = Quaternion.Lerp(transform.rotation, 
            Quaternion.Euler(transform.rotation.x, transform.rotation.y, _targetRotate.z), _rotateSpeed * Time.deltaTime);
    }
